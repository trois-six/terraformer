	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(bootstrapServers) == 0 {
				bootstrapServers = defaultKafkaBootStrapServer
			}
			...
			saslMechanism, present := os.LookupEnv("KAFKA_SASL_MECHANISM")
			if !present {
				saslMechanism = "plain"
			}
			...
			default:
				return fmt.Errorf("Invalid sasl mechanism \"%s\": can only be \"scram-sha256\", \"scram-sha512\" or \"plain\"", saslMechanism)
			}
			skipTLSVerify := os.Getenv("KAFKA_SKIP_VERIFY")
			tlsEnabled := os.Getenv("KAFKA_ENABLE_TLS")
